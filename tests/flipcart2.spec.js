...
test('flipkart add to cart', async ({ page }) => {
    ...
    // Click the 2nd mobile product using XPath
    ...
    if (!priceText) {
        const allText = await page.locator('text=/₹[\\d,]+/').first().textContent();
        priceText = allText || '';
    ...
    await page.waitForTimeout(2000); // Wait for toast notification
